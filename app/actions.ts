...
export async function analyzePdf(formData: FormData) {
  try {
    ...
    const { text } = await generateText({
      model: openai("gpt-4o"),
      prompt: `Please analyze this grading permit application section for any compliance or regulatory issues. Identify potential problems, flag them, and provide explanations for each issue.
...
{
  "complianceScore": number, // A score from 0-100 indicating overall compliance
  "grammarIssues": [
    {
      "quote": "string", // The exact text with the grammar issue
      "explanation": "string" // Explanation of the grammar issue
    }
  ],
  "ambiguityIssues": [
    {
      "quote": "string", // The exact text with ambiguity
      "explanation": "string" // Explanation of why this is ambiguous
    }
  ],
  "complianceIssues": [
    {
      "quote": "string", // The exact text with compliance issue
      "explanation": "string" // Explanation of the compliance issue
    }
  ]
}`,
    })
    ...
    try {
      ...
      return {
        ...
        complianceIssues: [
          {
            quote: "Analysis Error",
            explanation: "Could not process the analysis results. Please try again.",
          },
        ],
      }
    }
      ...
      complianceIssues: [
        {
          ...
          explanation: "Failed to process the PDF. Please ensure it's a valid document and try again.",
        },
      ],
    }
  }
